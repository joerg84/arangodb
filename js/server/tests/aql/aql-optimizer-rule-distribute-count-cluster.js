...
function optimizerRuleTestSuite () {
  ...
  return {
    ...
    testRulesNone : function () {
      var queries = [ 
        "FOR d IN " + cn1 + " COLLECT h = d.Hallo WITH COUNT INTO co RETURN {co}"
      ];
      queries.forEach(function(query) {
        var plan = AQL_EXPLAIN(query, { }, rulesNone);
        var result = helper.getQueryResults(query);
        print(JSON.stringify(plan.plan.nodes.map(function (node) { return node.type; }) ));
        print(JSON.stringify(result ));

        assertEqual([ "scatter-in-cluster" ], plan.plan.rules, query);
    ...
    testRulesDoesNotMatch : function () {
      var queries = [ 
        "FOR d IN " + cn1 + " COLLECT h = d.Hallo WITH COUNT INTO co RETURN {h, co}"
      ];
      queries.forEach(function(query) {
        var plan = AQL_EXPLAIN(query, { }, rulesNone);
        var result = helper.getQueryResults(query);
